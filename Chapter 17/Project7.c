/* Prints a one-month reminder list (dynamically allocated version) */
...
int main(void)
{
  ...
  for (;;)
  {
    ...
    reminders[i] = malloc(sizeof(struct vstring) + 2/*day_str*/ + strlen(msg_str) + 1/*null char*/);
    if (reminders[i] == NULL)
    {
      printf("-- No space left --");
      break;
  ...
  printf("\nDay Reminder\n");
  for(i = 0; i < num_remind; i++)
    printf(" %s\tStrlen: %d\n", reminders[i]->chars, reminders[i]->len);
